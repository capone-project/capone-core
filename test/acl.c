...
static struct sd_acl acl;
static struct sd_sign_key_public key1, key2;
...
{
    assert_success(sd_acl_add_right(&acl,&key1, SD_ACL_RIGHT_EXEC));
}

static void adding_entry_twice_fails()
{
    assert_success(sd_acl_add_right(&acl, &key1, SD_ACL_RIGHT_EXEC));
    assert_failure(sd_acl_add_right(&acl, &key1, SD_ACL_RIGHT_EXEC));
}

static void adding_entry_with_different_rights_works()
{
    assert_success(sd_acl_add_right(&acl, &key1, SD_ACL_RIGHT_EXEC));
    assert_success(sd_acl_add_right(&acl, &key1, SD_ACL_RIGHT_TERMINATE));
}

static void adding_entry_with_different_keys_works()
{
    assert_success(sd_acl_add_right(&acl, &key1, SD_ACL_RIGHT_EXEC));
    assert_success(sd_acl_add_right(&acl, &key2, SD_ACL_RIGHT_EXEC));
}

static void adding_entry_allows()
{
    assert_success(sd_acl_add_right(&acl, &key1, SD_ACL_RIGHT_EXEC));
    assert_true(sd_acl_is_allowed(&acl, &key1, SD_ACL_RIGHT_EXEC));
}
...
{
    assert_success(sd_acl_add_wildcard(&acl, SD_ACL_RIGHT_EXEC));
    assert_true(sd_acl_is_allowed(&acl, &key1, SD_ACL_RIGHT_EXEC));
}
...
{
    assert_success(sd_acl_add_wildcard(&acl, SD_ACL_RIGHT_EXEC));
    assert_false(sd_acl_is_allowed(&acl, &key1, SD_ACL_RIGHT_TERMINATE));
}

static void removing_nonexistent_entry_fails()
{
    assert_failure(sd_acl_remove_right(&acl, &key1, SD_ACL_RIGHT_EXEC));
}

static void removing_entry_removes_right()
{
    assert_success(sd_acl_add_right(&acl, &key1, SD_ACL_RIGHT_EXEC));
    assert_success(sd_acl_remove_right(&acl, &key1, SD_ACL_RIGHT_EXEC));
    assert_false(sd_acl_is_allowed(&acl, &key1, SD_ACL_RIGHT_EXEC));
}
...
static void removing_entry_does_not_remove_other_rights()
{
    ...
    assert_success(sd_acl_add_right(&acl, &key1, SD_ACL_RIGHT_TERMINATE));
    assert_success(sd_acl_remove_right(&acl, &key1, SD_ACL_RIGHT_EXEC));
    assert_true(sd_acl_is_allowed(&acl, &key1, SD_ACL_RIGHT_TERMINATE));
}
...
static void removing_entry_does_not_remove_other_keys()
{
    ...
    assert_success(sd_acl_add_right(&acl, &key2, SD_ACL_RIGHT_EXEC));
    assert_success(sd_acl_remove_right(&acl, &key1, SD_ACL_RIGHT_EXEC));
    assert_true(sd_acl_is_allowed(&acl, &key2, SD_ACL_RIGHT_EXEC));
}

static void empty_acl_allows_nothing()
{
    assert_success(sd_acl_is_allowed(&acl, &key1, SD_ACL_RIGHT_TERMINATE));
}
