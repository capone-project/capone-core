...
do
    ...
    do
        PLAIN=$(./build/cpn-bench-throughput --plain ${datalen} ${pkglen} | awk '!(NR%2) {printf "%s %s", $NF, p} {p=$NF}')
        ...
        ENCRYPTED=$(./build/cpn-bench-throughput --encrypted ${datalen} ${pkglen} | awk '!(NR%2) {printf "%s %s", $NF, p} {p=$NF}')
