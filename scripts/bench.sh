...
do
    ...
    do
        PLAIN=$(./source/sd/build/sd-bench-throughput --plain ${datalen} ${pkglen} | awk '!(NR%2) {printf "%s %s", $NF, p} {p=$NF}')
        ...
        ENCRYPTED=$(./source/sd/build/sd-bench-throughput --encrypted ${datalen} ${pkglen} | awk '!(NR%2) {printf "%s %s", $NF, p} {p=$NF}')
